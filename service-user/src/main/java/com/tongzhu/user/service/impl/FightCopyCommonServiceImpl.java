...
@Service
public class FightCopyCommonServiceImpl implements FightCopyCommonService {

    private Logger logger= LoggerFactory.getLogger(FightCopyCommonServiceImpl.class);
    ...
    public int useBigAndSkill(Combatant peerA, Combatant peerB, List<FightStepDetailVO> resultData,
                              FightParameter fightParameter,List<Combatant> groupB,
                              Map<String,Integer> awardMap,List<Map<String,Object>> defendGroup) {
        float A1=40;
        float B1=40;
        float C1=55;
        //使用大招，技能
            ...
            if((userSkills!=null && !userSkills.isEmpty()) && bloodRate<0.3 && RandomUtil.probabilityEvent(A1)){
                ...
                if(setting!=null && StringUtils.isNotBlank(setting.getPro())){
                    ...
                    if(object.getFloat(BuffConstant.BUFF_1017)!=null){ //1017 1018在同一技能出现
                        int blood=peerA.getBlood();
                        int addBlood=(int)(blood*object.getBigDecimal(BuffConstant.BUFF_1017).floatValue()/100f);
                        int bloodCount=addBlood;
                        fightParameter.setBloodCount(bloodCount);
                    ...
                    if((skillList!=null && !skillList.isEmpty()) && RandomUtil.probabilityEvent(C1)) {
                        ...
                        if (canRelease) {
                            ...
                            if(peerB.getGroup().equals(1)){
                                this.releaseSkill(peerA,peerB,skill,fightParameter,resultData,groupB);
                                int computeResult=this.computeResult(peerA,peerB,resultData,fightParameter);
                                if(computeResult<3){
                                    return computeResult;
                                }
                            }
                            if(peerB.getGroup().equals(2)){
                                int i=0;
                                while (!CollectionUtils.isEmpty(groupB.get(0).getCombatantList()) && i<groupB.get(0).getCombatantList().size()){
                                    this.releaseSkill(peerA,groupB.get(0).getCombatantList().get(i),skill,fightParameter,resultData,groupB);
                                    int computeResult=this.computeResult(peerA,groupB.get(0).getCombatantList().get(i),resultData,fightParameter);
                                    int tempi=i;
                                    i++;
                                    if(computeResult==1){//玩家赢了，怪被kill
                                        i=tempi;
                                        logger.info("怪物{}[{}]被kill掉了一只",groupB.get(0).getCombatantList().get(tempi).getUserName(),groupB.get(0).getCombatantList().get(tempi).getGroupIndex());

                                        FightStepDetailVO stepDetail1=new FightStepDetailVO();
                                        stepDetail1.setType(FightingConstant.FIGHT_STEP_TYPE_99);
                                        Map<String,Object> deadPlayer=new HashMap<>();
                                        deadPlayer.put("group",groupB.get(0).getCombatantList().get(tempi).getGroup());
                                        deadPlayer.put("index",groupB.get(0).getCombatantList().get(tempi).getGroupIndex());
                                        ...
                                        removeCombatant(groupB.get(0).getCombatantList(),tempi);
                                    }
                                }
                                if(groupB.get(0).getCombatantList().isEmpty()){
                                    ...
                                    if(!CollectionUtils.isEmpty(groupB)){
                                        ...
                                        stepDetail.setSelfIndex(peerA.getGroupIndex());
                                        stepDetail.setOpponentIndex(groupB.get(0).getGroupIndex());
                                        resultData.add(stepDetail);
                                    }
                                }
                                if(!CollectionUtils.isEmpty(groupB) && !CollectionUtils.isEmpty(groupB.get(0).getCombatantList())){
                                    this.getCombatant(defendGroup,groupB.get(0));
                                }
                            }
                        }else {
                            //没有释放主动技能，则使用武器攻击
    ...
    @Override
    public int useWeapon(Combatant peerA, Combatant peerB, List<FightStepDetailVO> resultData,FightParameter fightParameter,List<Combatant> groupB) {
        ...
        if(peerA.getDebuff()!=null && peerA.getDebuff().containsKey(BuffConstant.BUFF_1011) &&
            ...
            if(arsenalInfoVOList!=null && !arsenalInfoVOList.isEmpty() && rand<=D1*10){
            ...
            }else if(arsenalInfoVOList!=null && !arsenalInfoVOList.isEmpty() && rand>D1*10 && rand<=(D1+D2)*10){
            ...
            if((peerA.getDebuff()!=null && peerA.getDebuff().containsKey(BuffConstant.BUFF_1021) &&
                ...
                if(peerB.getDebuff()!=null && peerB.getDebuff().containsKey(BuffConstant.BUFF_1020) &&
                    ...
                    //初始暴击伤害比例 150%
                    int hurtCount=(int)(fightParameter.getHurtCount()*1.5);
                    fightParameter.setHurtCount(hurtCount);
                    FightStepDetailVO stepDetail=new FightStepDetailVO();
                    switch (fightParameter.getType()){
                        case 1://徒手进行攻击
    ...
    public List<Combatant> getCombatant(List<Combatant> fightGroup) {
        ...
        for(Combatant combatant:fightGroup){
            ...
            if(combatant.getWeaponList()!=null && !combatant.getWeaponList().isEmpty()){
                combatant.getWeaponList().forEach((ArsenalInfoVO arsenalInfoVO)->{
    ...
    @Override
    public void getCombatant(List<Map<String,Object>> fightGroup, Combatant combatant) {
        Map<String,Object> monsterMap=new HashMap<>();
    ...
    public int computeResult(Combatant peerA, Combatant peerB, List<FightStepDetailVO> resultData,FightParameter fightParameter) {
        ...
        if(peerB.getBuff()!=null && peerB.getBuff().containsKey(BuffConstant.BUFF_1010) &&
            ...
            Float rate=(peerB.getBuff().get(BuffConstant.BUFF_1010).get("rate")==null?0f:peerB.getBuff().get(BuffConstant.BUFF_1010).get("rate"));
            int hurtCount=fightParameter.getHurtCount()-(int)(fightParameter.getHurtCount()*(rate/100));
            fightParameter.setHurtCount(hurtCount);
            FightStepDetailVO stepDetail=new FightStepDetailVO();
        ...
        if(fightParameter.getType()!=null && fightParameter.getType()>0 && peerB.getBuff()!=null && peerB.getBuff().containsKey(BuffConstant.BUFF_1046) &&
